# GREEN-AI GOVERNANCE SYSTEM — MASTER SPEC v2 (PRODUCTION)
...
| `FlakeRetried` | bool      | ❌   | false   | True = passed only after retry |
...
2. critical persists (iteration ≥ 2) → STOP
3. no progress ×2 → STOP
4. score ≥ 80 AND no critical → SUCCESS
...
| Status | Condition |
|--------|-----------|
| `engine-fault` | Failed rule has no Message |
| `suspect` | Passed rule has unexpectedly high ExecutionMs |
| `flaky` | `FlakeRetried = true` |
| `meta` | `IsMeta = true` — skipped entirely |
...
```json
{
  "_governance": {
    "version": "1.0.0",
    "reviewedBy": "AI",
    "note": "Thresholds may only be tightened — never loosened to hide violations"
  },
  "overflowThresholdPx": 2,
  "textOverflowThresholdPx": 4,
  "topBarThresholdPx": 4
}
```
...
Console audit trail emitted on startup: `[GovernanceRunner] threshold version: 1.0.0`
...
* Chromium = baseline
* Firefox/WebKit = validation only
...
**`governance-report.json`** — written after every test run:

* `score`, `dimensions`, `rootCauses`, `coverage`
* `visualConsistency` (meta — never scored)
* `performance` (DOMContentLoaded, TTI, layoutShift — meta)
* `stability` (flakeRetried counts)
* `validations` (from `RuleValidator`)

**`governance-delta.json`** — written after every run, used by loop stop conditions:

* `previousScore`, `currentScore`, `scoreDelta`, `isRegression`

**`analysis-pack.zip`** — produced by `VisualAnalysisExporter` + `ExportVisualAnalysisTests`:

* Packages `TestResults/Visual/current/{device}/*.png` into a ZIP
* Includes `instructions.json` for external AI upload (ChatGPT, Claude)
* Device folders: `desktop/`, `laptop/`, `tablet/`, `mobile/`
...
```
flakyCount     = results.Count(r => r.FlakeRetried)
stabilityScore = results.Count == 0 ? 1.0 : Round(1.0 - flakyCount / results.Count, 2)
```

**Freeze gate:** `stabilityScore == 1.0` required at activation.

Used for: freeze gate, trend analysis, regression detection
